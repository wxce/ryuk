...
class PaginatorButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.button_type == "first":
            ...
            if self.paginator.disable_on_timeout:
              for item in self.paginator.children:
                item.disabled = True
              return await self.paginator.message.edit(view=self.paginator)
...
class Paginator(discord.ui.View):
    ...
    def add_default_buttons(self):
        default_buttons = [
            PaginatorButton("prev", emoji="◀️", style=discord.ButtonStyle.blurple),
            PaginatorButton("stop", emoji="⏹️", style=discord.ButtonStyle.blurple),
            PaginatorButton("next", emoji="▶️", style=discord.ButtonStyle.blurple),
    ...
    def add_button(self, button: PaginatorButton):
        self.buttons[button.button_type] = {
            "object": discord.ui.Button(
                style=button.style,
                label=f"{self.current_page + 1}/{self.page_count + 1}" if button.label == "page_indicator" else None,
