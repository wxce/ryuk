...
from config import ryuk_GUILD_ID, STAFF_ROLE, OWNER_ROLE
import discord
import json
...
class InviteTracking(commands.Cog):
    ...
    async def check_staff(self, user: Union[discord.Member, discord.User]):
        ryuk_guild = self.client.get_guild(ryuk_GUILD_ID)
        if ryuk_guild not in user.mutual_guilds:
            return False
        member = ryuk_guild.get_member(user.id)
        if ryuk_guild.get_role(OWNER_ROLE) in member.roles:
            return "owner"
        elif ryuk_guild.get_role(STAFF_ROLE) in member.roles:
            return "staff"
        return False
    ...
    async def welcome_msg(self, member: discord.Member):
        ...
        if not staff:
            ...
            if staff == 'owner':
                ...
                await channel.send(f"{member.mention} is an ryuk staff member 😊")
        return
    ...
    async def autorole(self, member: discord.Member):
        ...
        for e in guild_config['autorole']['all']:
            await member.add_roles(member.guild.get_role(e), reason="ryuk autorole")
