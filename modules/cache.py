...
class Cache:
    ...
    async def initialize_settings_cache(self):
        ...
        prefixes = await self.bot.db2.execute("SELECT guild_id, prefix FROM guild_prefix")
        for guild_id, prefix in prefixes:
            self.prefixes[str(guild_id)] = prefix

        self.rolepickers = set(
            await self.bot.db2.execute("SELECT channel_id FROM rolepicker_settings", as_list=True)
        )

        self.votechannels = set(
            await self.bot.db2.execute("SELECT channel_id FROM voting_channel", as_list=True)
        )

        guild_settings = await self.bot.db2.execute(
        ...
        self.blacklist = {
            "global": {
                "user": set(
                    await self.bot.db2.execute("SELECT user_id FROM blacklisted_user", as_list=True)
                ),
                "guild": set(
                    await self.bot.db2.execute(
        ...
        self.marriages = [
            set(pair)
            for pair in await self.bot.db2.execute(
                "SELECT first_user_id, second_user_id FROM marriage"
            )
        ]

        for guild_id, user_id in await self.bot.db2.execute(
        ...
        for guild_id, command_name in await self.bot.db2.execute(
