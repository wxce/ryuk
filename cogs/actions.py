...
class actions(commands.Cog, description="Interact with someone, ~"):
    ...
    async def make_actions_msg(self, author, ctx, error_msgs, embed_stuff, url, user: discord.Member = None):
        if user is None:
            ...
            return error_embed("error", error_msgs[0])
        if user == author:
            ctx.command.reset_cooldown(ctx)
            return error_embed("error", error_msgs[1])
        if user == self.client.user:
            ctx.command.reset_cooldown(ctx)
            return error_embed("error", error_msgs[2])
        embed = discord.Embed(
            title=embed_stuff[0],
