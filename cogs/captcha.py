...
class Join(commands.Cog):
    ...
    async def verify(self, ctx):
        ...
        if checkrole not in ctx.author.roles:
            ...
            font = ImageFont.truetype(font="Tools/arial.ttf", size=70)

            text = ' '.join(
                random.choice(string.ascii_uppercase) for _ in range(5))  # + string.ascii_lowercase + string.digits
            ...
            p.random_distortion(probability=1, grid_width=5, grid_height=5, magnitude=15)
            p.process()
            ...
            try:
                ...
                captcha_embed = discord.Embed(title=f"{member.guild.name} Captcha Verification",
                                              description=f"{member.mention} Please return the code written on the Captcha.",
                                              color=0xb0a0a0)
                captcha_embed.set_image(url="attachment://captcha.png")
                captcha_embed.set_footer(text=f"want this bot in your server? → ;invite")
            ...
            try:
                ...
                if msg.content == password:
                    ...
                    embed = discord.Embed(
                        ...
                        description="User {0} has successfully passed the captcha.".format(member.mention), color=0xb0a0a0)
                    embed.add_field(
                        name="User ID",
                    ...
                    if captchapunishment == "kick":
                        try:
                            await member.kick(reason="Ryuk Captcha Protection | User failed the captcha")
                        except:
                            pass

                    if captchapunishment == "ban":
                        try:
                            await member.ban(reason="Ryuk Captcha Protection | User failed the captcha")
                        except:
                            pass
                ...
                embed = discord.Embed(
                    ...
                    timestamp=datetime.now().astimezone(tz=us), color=0xb0a0a0)
                embed.add_field(
                    name="User ID",
                    value=f'{member.id}', inline=False)
                embed.add_field(name="Date", value=f"<t:{now}:F>\n", inline=False)
