...
class typing(commands.Cog):
    ...
    async def typing_race(self, ctx, language=None, wordcount: int = 25):
        ...
        await self.bot.db2.executemany(
            """
    ...
    async def typing_clear(self, ctx):
        ...
        async def confirm():
            await self.bot.db2.execute("DELETE FROM typing_stats WHERE user_id = %s", ctx.author.id)
            await self.bot.db2.execute("DELETE FROM typing_race WHERE user_id = %s", ctx.author.id)
