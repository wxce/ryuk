...
Example: `{prefix}shouldi vote ryuk OR invite ryuk`
...
Another Example: `{prefix}shouldi Study OR Procrastinate`
    ...
    async def brainsize(self, ctx, user: Union[discord.Member, str] = None):
        ...
        if iq == 0:
            ...
            color = MAIN_COLOR
            footer = "damn ur a genius! not as smart as me though" 
            size = "big"
        embed = discord.Embed(
            title="🧠 your IQ",
    ...
    @commands.command(aliases=['firstmsg', 'firstmessage', 'first_msg'], help="Get the first message of the channel.")
    ...
    async def whendie(self, ctx, *, user: discord.Member = None):
        ...
        if thingy == something[5]:
            funny_text = "Wowie, you have a nice long life! "
    ...
    async def simp_for(self, ctx: commands.Context, user: discord.Member = None):
        if user is None:
            ctx.command.reset_cooldown(ctx)
            return await ctx.reply(embed=error_embed("error", "please mention someone to simp for"))
        if user == ctx.author:
            ctx.command.reset_cooldown(ctx)
            return await ctx.reply(embed=error_embed(
...
**{escape_markdown(ctx.author.name)}** is now simping for **{escape_markdown(user.name)}**.
    ...
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.command(help="Make your text cuter!")
    ...
    async def say(self, ctx, *, message=None):
        if message is None:
            ctx.command.reset_cooldown(ctx)
            return await ctx.message.reply(embed=error_embed("error", "please enter something to say next time!"))
        for word in DEFAULT_BANNED_WORDS:
    ...
    async def predict(self, ctx, *, question=None):
        if question is None:
            ctx.command.reset_cooldown(ctx)
            return await ctx.message.reply(embed=error_embed("error", "please enter a question!"))
    ...
    async def hack(self, ctx: commands.Context, user: discord.Member = None):
        if user is None:
            ctx.command.reset_cooldown(ctx)
            return await ctx.message.reply(embed=error_embed("error", "please mention who do you want to hack next time!"))

        if user == ctx.author:
            ctx.command.reset_cooldown(ctx)
            return await ctx.message.reply(embed=error_embed("error", "Don't hack yourself idiot!"))
        if user.bot:
            ctx.command.reset_cooldown(ctx)
            return await ctx.message.reply(embed=error_embed("error", "you can't hack bots.\nThey are way too powerful!"))
